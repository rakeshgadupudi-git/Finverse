...
import { getNewsByTicker } from './newsData.js';
...
function generateSentiment(stock) {
  ...
  const socialScore = Math.round(positiveNews / totalNews * 60 + (stock.changePercent > 0 ? 20 : 5) + Math.random() * 15);
  const analystRating = Math.round((stock.roe / 5 + stock.revenueGrowth / 3 + (100 - (stock.pe || 50)) / 10) * 10) / 10;
  const confidenceScore = Math.min(95, Math.max(30, Math.round(socialScore * 0.4 + analystRating * 3 + Math.random() * 10)));

  const totalAnalysts = Math.floor(Math.random() * 20) + 15;
